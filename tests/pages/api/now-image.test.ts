...
describe('GET /api/now-images/:imageId', () => {
  ...
  it('只代理已发布图片并设置缓存头，响应不泄露 Bot Token', async () => {
    ...
    vi.spyOn(globalThis, 'fetch')
      ...
      .mockResolvedValueOnce(new Response(new Uint8Array([1, 2, 3]), { headers: { 'content-type': 'image/jpeg', 'content-length': '3' } }));
  ...
  it('将 Telegram 失败、不安全类型和超大响应安全地拒绝', async () => {
    ...
    vi.mocked(fetch)
      ...
      .mockResolvedValueOnce(new Response(null, { headers: { 'content-type': 'image/jpeg', 'content-length': String(10 * 1024 * 1024 + 1) } }));
    expect((await getImage('public-image-id')).status).toBe(502);

    vi.mocked(fetch)
      .mockResolvedValueOnce(Response.json({ ok: true, result: { file_path: 'missing.jpg' } }))
      .mockResolvedValueOnce(new Response('missing', { status: 404, headers: { 'content-type': 'image/jpeg' } }));
    expect((await getImage('public-image-id')).status).toBe(502);
